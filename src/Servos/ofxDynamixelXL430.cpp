...
add<uint8_t>(  torqueEnabled          , 64  , "torqueEnable"          , 0     , 0     , 1     , false , false );  // "Motor Torque On/Off                              | RW | 0     |
add<uint8_t>(  led                   , 65  , "led"                   , 0     , 0     , 1     , false , false );  // "Status LED On/Off                                | RW | 0     |
add<uint8_t>(  statusReturnLevel     , 68  , "statusReturnLevel"     , 2     , 0     , 2     , false , false );  // "Select Types of Status Return                    | RW | 2     |
add<uint8_t>(  registeredInstruction , 69  , "registeredInstruction" , 0     , 0     , 1     , true  , false );  // "Check Reception of Instruction                   | R  | 0     |
add<uint8_t>(  hardwareErrorStatus   , 70  , "hardwareErrorStatus"   , 0     , 0     , 1     , true  , false );  // "Hardware Error Status                            | R  | 0     |//BITFIELD
...
add<uint8_t>(  moving                , 122 , "moving"                , 0     , 0     , 1     , true  , false );  // "Movement Status                                  | R  | 0     |
